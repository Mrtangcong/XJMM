<template>
  <div>
    <div class="tage">
         <Tag class="rengong" :row-class-name="rowClassName" v-if="show" closable @on-close="handleClose">历史打款记录</Tag>
    </div>
    ...
    <div class="risk">
        ...
        <Row :gutter="16">        
          ...
          <Col span="21">
            <div class="equal">   
             ...
             <div class="equal">
              <Button type="ghost" style="margin-right:-5px">放款状态</Button>
              <Select clearable v-model="mloan" style="width:170px" size="large">
                <Option v-for="item in loanStatus" :value="item.value" :key="item.value">{{ item.label }}</Option>
              </Select>

              <Button type="ghost" class="btn-sta">到账状态</Button>
              <Select clearable v-model="marrival" style="width:170px" size="large">
                <Option v-for="item in arrivalStatus" :value="item.value" :key="item.value">{{ item.label }}</Option>
              </Select>              
            </div> 
            <div class="equal">
      ...
      <div id="rpage">
        <Page :total="yeshu" size="small" show-sizer></Page>
      </div>
    </div>
    <!-- modal -->
...
export default {
  ...
  mounted(){
    var that =this
    this.getdata();
    axios.post('http://192.168.0.105:8081/payment/getAllPayRecords',{})
            .then(function(res){
                console.log(res)
                that.lishi =res.data.data 
                that.yeshu = res.data.totalPage*10
                console.log(that.yeshu)
            })
  ...
  methods: {
    ...
    searchTable(){
      ...
      axios.post('http://192.168.0.105:8081/payment/getAllPayRecords',{"startTime":ac,"endTime":ac1,"operator":that.orderNum,"payStatus":that.mloan,"transferStatus":that.marrival})
            .then(function(res){
                console.log(res)
                that.lishi =res.data.data
