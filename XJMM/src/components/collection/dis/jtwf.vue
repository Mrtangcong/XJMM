  ...
  <div>
    <div class="work">
      ...
      <div class="equal">
        ...
        <Button type="primary" slot="extra">分配案件</Button>
      </div>
      <div id="rdate">
    ...
    <!-- modal -->
      <Modal v-model="orderModal"  title="订单详情" width="780px" :mask-closable="false">
          <div>订单详情</div>
          <div class="box">
      ...
      <Modal v-model="assigncaseModal1" title="分配案件" :mask-closable="false">
        <div class="box1">
          <div>
         ...
         <div>
            <Table :columns="distribute" :data="distributeData1"></Table>
          </div>
        </div>
        <div slot="footer">
            <Button type="warning" @click="cancel1">取消</Button>
            <Button type="primary" @click="yes1">确认转入</Button>
        </div>
      </Modal>
      <Modal v-model="assigncaseModal2" title="一键均分案件" :mask-closable="false">
        <div class="box1">
          <div>
            <Button type="text" >选择催收人员</Button>
            <Select clearable v-model="mcuiPerson" style="width:200px" size="large">
          ...
          <div class="h1" style="margin:20px 0">分配概览</div>
         <div>
            <Table :columns=" distribute" :data="distributeData2"></Table>
          </div>
        </div>
        <div slot="footer">
            <Button type="warning" @click="cancel2">取消</Button>
            <Button type="primary" @click="yes2">一键均分</Button>
        </div>
      </Modal>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      assigncaseModal1:false,//分配案件
      assigncaseModal2:false,//一键均分
          ...
          render: (h, params) => {
            return h('Select', [
             ...
             h('Button', {
                ...
                on: {
                  click: () => {
                    this.showAssigncase(params.row.orderNo)
                  }
                }
              },'分配案件'),
              h('Button', {
  ...
  methods: {
    ...
    showAssigncase(index){//分配案件
      this.assigncaseModal1=true;
    },
    cancel1(){
      this.assigncaseModal1=false; 
    },
    yes1(){
      this.assigncaseModal1=false;
    },
    option(value){//批量操作 一键均分
      if(value == 1){
        this.assigncaseModal2=true;        
      }
    },
    cancel2(){
    ...
    searchTable(){  //我的工作台查询
       var that = this;
      axios.post('http://192.168.0.188:8088/due/casedivide/todayNotDivide', {
        
         "caseStatus":that.mcaseStatus,
         "name": that.name,
